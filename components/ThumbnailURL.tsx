import React from 'react';

type Props = {
  url: string;
};

const ThumbnailURL: React.FC<Props> = ({ url }) => {
  return (
    <div className="p-[1px] inline-block bg-gradient-to-b from-[rgba(108,123,138,1)] to-[rgba(121,121,121,0.2)] rounded-md shadow-[0px_4px_4px_rgba(0,0,0,0.25)]">
      <div className="rounded-md bg-dark flex w-full items-center justify-start space-x-2 pr-2">
        <div className="flex w-full space-x-2">
          <div className="flex py-1 px-2 rounded-l-md bg-gradient-to-b from-[rgba(240,240,240,0.32)] to-[rgba(105,105,105,0.14)] text-sm select-none">Video Thumbnail</div>
          <input
            readOnly
            className="bg-transparent outline-none text-xs flex flex-grow"
            onClick={(e: React.MouseEvent<HTMLInputElement>) => {
              let input = e.target as HTMLInputElement;
              input.select();
            }}
            type="text"
            defaultValue={url}
          />
        </div>
      </div>
    </div>
  );
};
export default ThumbnailURL;
